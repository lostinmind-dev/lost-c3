import type { BaseConfig } from '../addon/config/index.ts';

const config: BaseConfig = {} as BaseConfig
...
const PLUGIN_CLASS = SDK.Plugins[config.info.id] = class LostC3 extends SDK.IPluginBase {
    constructor() {
        ...
        if (info.type === 'world') {
            ...
            if (typeof info.mustPreDraw !== 'undefined') {
    ...
    private init() {
        ...
        this._info.SetProperties(config.properties.flatMap(property => {
            if (property.type === 'integer' || property.type === 'float') {
                ...
                return new SDK.PluginProperty(property.type, property.id, property.value);
